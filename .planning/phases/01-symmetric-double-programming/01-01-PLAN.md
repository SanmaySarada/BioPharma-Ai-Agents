...
must_haves:
  truths:
    - "Downstream plans can import TrackResult, StageComparison, ResolutionHint, and ResolutionResult from omni_agents.models.resolution"
    - "StageComparator (Plan 02) can construct StageComparison with per-stage comparison results and an issues list"
    - "ResolutionLoop (Plan 04) can call ResolutionHint.to_prompt_text() to produce structured feedback for agents"
    - "Orchestrator (Plan 03) can read ResolutionConfig.enabled and .max_iterations from Settings"
    - "ScriptCache.cache_key('track_a') and cache_key('track_b') produce different keys for the same agent, preventing cross-track collisions"
  artifacts:
