...
Body: Directly adapt `_run_track_a`. Key changes:
- Replace hardcoded "track_a" with `track_id` parameter everywhere.
- Replace hardcoded `gemini` with `llm` parameter.
- Track-qualify callback step_names: use `f"{stage}_{track_id}"` format (e.g., "sdtm_track_a", "adam_track_b"). This makes parallel callbacks distinguishable.
- Track-qualify state step names: use `f"{stage}_{track_id}"` (e.g., "sdtm_track_a").
- Pass `track_id=track_id` to `self._run_agent()`.
- Return `TrackResult(track_id=track_id, sdtm_dir=sdtm_dir, adam_dir=adam_dir, stats_dir=stats_dir, results_path=stats_dir / "results.json")`.

The SDTM, ADaM, Stats agent instantiation remains the same pattern (each agent takes `llm=llm, prompt_dir=prompt_dir, trial_config=self.settings.trial`).

Schema validation remains the same (SchemaValidator.validate_sdtm, validate_adam, validate_stats).

Important: Do NOT import or use `BaseLLM` for type annotation if it creates a circular import. Instead use the existing import pattern -- the `GeminiAdapter` and `OpenAIAdapter` are already imported. If needed, use `from omni_agents.llm.base import BaseLLM` which is the existing base class.
...
Also verify: `python -c "from omni_agents.pipeline.consensus import ConsensusJudge; print(hasattr(ConsensusJudge, 'compare_symmetric'))"` -- should print True.
  ...
  <done>Orchestrator has single _run_track method (no _run_track_a or _run_track_b). _run_track accepts track_id and BaseLLM, returns TrackResult. _run_agent passes track_id to cache_key. run() calls _run_track("track_a", gemini, ...) and _run_track("track_b", openai, ...) in parallel. ConsensusJudge.compare_symmetric reads same-format results.json from both tracks.</done>
</task>
