...
must_haves:
  truths:
    ...
    - "Tests verify validate_output_completeness catches missing adam/ADSL.csv"
    - "All existing tests still pass (no regressions)"
  artifacts:
    - path: "tests/test_pipeline/test_schema_validator.py"
      provides: "ADSL validation tests and updated output completeness tests"
      contains: "adsl"
  ...
  key_links:
    ...
    - from: "tests/test_pipeline/test_schema_validator.py"
      to: "src/omni_agents/pipeline/schema_validator.py"
      via: "import SchemaValidator"
      pattern: "SchemaValidator.validate_output_completeness"
...
<objective>
Add tests for ADSL validation, ADSL data dictionary entries, updated output completeness checks, and updated ADaM agent prompt.
...
<task type="auto">
  <name>Task 1: Add ADSL schema validation tests and update output completeness tests</name>
  <files>tests/test_pipeline/test_schema_validator.py</files>
  <action>
  **Part A: Update _make_adam_dir helper for ADSL support.**
  ...
  **Part B: Add ADSL validate_adam tests.**
  ...
  **Part C: Update output completeness tests for ADSL.csv.**

  5. Update the existing `test_output_completeness_passes_with_both_dicts` test to ALSO create `adam/ADSL.csv` in the fixture (since `validate_output_completeness()` now checks for it). Rename to `test_output_completeness_passes_with_dicts_and_adsl`:
  ...
  6. Add a new test for missing ADSL.csv in output completeness:
  ...
  7. Update `test_output_completeness_fails_missing_both` to also account for ADSL.csv being missing (the issue count should now be 3 instead of 2, since ADSL.csv is also missing). Or keep it at 2 by creating ADSL.csv in the fixture -- use whichever accurately reflects the updated `validate_output_completeness()` behavior. If both dicts are missing AND ADSL.csv is missing, the error should have 3 issues.
  </action>
  <verify>Run `python -m pytest tests/test_pipeline/test_schema_validator.py -v` -- all tests pass including new ADSL tests and updated output completeness tests.</verify>
  <done>5 new ADSL validation tests pass. 1 new output completeness test for missing ADSL. Existing output completeness tests updated for ADSL. 3 existing ADTTE tests still pass.</done>
</task>
...
<verification>
- `python -m pytest tests/test_pipeline/test_schema_validator.py -v` -- all pass (14+ tests including output completeness)
...
- 1 new output completeness test for missing ADSL.csv
- Existing output completeness tests updated to include ADSL.csv in fixtures
