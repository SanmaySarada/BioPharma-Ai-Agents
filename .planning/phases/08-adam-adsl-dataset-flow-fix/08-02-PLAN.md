...
<task type="auto">
  ...
  **Structure of the new template:**
  ...
  4. **NEW: Part 1 -- ADSL Dataset** section. Explain ADSL construction:
     ...
     **ADSL must be created FIRST, before ADTTE.** The R code should:

     a. Read DM.csv and VS.csv
     b. Create ADSL with one row per subject containing:
        - `STUDYID`: "{{ study_id }}"
        - `USUBJID`: from DM
        - `SUBJID`: from DM
        - `AGE`: from DM
        - `AGEU`: "YEARS"
        - `AGEGR1`: "<65" if AGE < 65, ">=65" otherwise
        - `SEX`: from DM
        - `RACE`: from DM
        - `ARM`: from DM
        - `ARMCD`: from DM
        - `TRT01P`: = ARM (no crossover)
        - `TRT01A`: = ARM (no crossover)
        - `SAFFL`: "Y" for all randomized subjects
        - `ITTFL`: "Y" for all randomized subjects
        - `EFFFL`: "Y" if subject has >= 1 post-baseline VS observation with non-NA VSSTRESN
        - `TRTSDT`: 0 (baseline week)
        - `TRTEDT`: last observed (non-NA) VISITNUM for the subject from VS. Use `max(VISITNUM[!is.na(VSSTRESN)])`.
          IMPORTANT: Handle with `na.rm = TRUE` AND check for empty vector -- `max(integer(0), na.rm=TRUE)` returns `-Inf`. Subjects with all NA should get TRTEDT = 0.
        - `TRTDUR`: TRTEDT - TRTSDT
        - `EOSSTT`: "COMPLETED" if TRTEDT == {{ visits - 1 }}, "DISCONTINUED" otherwise
        - `DCSREAS`: "" (empty string) if EOSSTT == "COMPLETED", "Dropout" if EOSSTT == "DISCONTINUED"
...
print('Template renders correctly with all required content')
  "` from the project root.
  </verify>
  <done>adam.j2 instructs LLM to generate R code that creates ADSL.csv first (with all 20 columns), then ADTTE from ADSL (not DM), producing 4 output files total.</done>
</task>
...
<verification>
- adam.j2 renders with ADSL construction instructions, population flags, disposition, treatment timing
...
- ADSL spec includes all 20 variables (demographics, treatment, population flags, disposition, timing)
